...
public class ShortestPaths<VertexType extends Vertex<EdgeType>, EdgeType extends Edge> {
    ...
    public PathfindingSnapshot extendSearch(int maxToSettle) {
        ...
        int settleCount = 0;
        while(!frontier.isEmpty() && settleCount < maxToSettle )
        {
            int currentVertexID = frontier.remove();
            Vertex<EdgeType> currentVertex = graph.getVertex(currentVertexID);
            for(EdgeType neighborEdge : currentVertex.outgoingEdges())
            {
                if(neighborEdge.endId() >= 0 && neighborEdge.endId() != startId)
                {
                    int dist = distances[currentVertexID]  + weigher.weight(neighborEdge);
                    if(predecessors[neighborEdge.endId()] == -1 || dist < distances[neighborEdge.endId()])
                    {
                        distances[neighborEdge.endId()] = dist;
                        predecessors[neighborEdge.endId()] = currentVertexID;
                        frontier.addOrUpdate(neighborEdge.endId(), dist);
                    }
                }
            }
