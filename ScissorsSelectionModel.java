...
public class ScissorsSelectionModel extends SelectionModel {
    ...
    protected void appendToSelection(Point p) {
        ...
        int id = graph.idAt(p);
        PolyLine line = graph.pathToPolyLine(paths.pathTo(id));
        controlPoints.add((Point) p.clone());
        segments.add(line);
        findPaths(id);
    }
    ...
    public PolyLine liveWire(Point p) {
        assert !controlPoints().isEmpty() && p != null;

        int id = graph.idAt(p);
        return graph.pathToPolyLine(paths.pathTo(id));
    }
    ...
    private class ShortestPathsWorker
        ...
        public PathfindingSnapshot doInBackground() {
            ...
            PathfindingSnapshot p = null;

            while (!pathfinder.allPathsFound()) {

                p = pathfinder.extendSearch(10000);
                this.setProgress((int) ((float) pathfinder.settledCount() / pathfinder.vertexCount() * 100));
                this.publish(p);
        ...
        public void done() {
            ...
            try {
                if (this.isCancelled() && state.equals(ScissorsState.ADDING)) {

                    if (controlPoints.size() == 1)
                        ScissorsSelectionModel.this.setState(ScissorsState.NO_SELECTION);
                    else
                        ScissorsSelectionModel.this.setState(ScissorsState.SELECTING);
            ...
            if (state.equals(ScissorsState.ADDING)) {
                ScissorsSelectionModel.this.setState(ScissorsState.SELECTING);
            } else if (state.equals(ScissorsState.MOVING)) {
                ScissorsSelectionModel.this.setState(ScissorsState.SELECTING);
            }
            worker = null;
