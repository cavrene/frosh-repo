...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
    ...
    private void paintSelectionPerimeter(Graphics g, List<PolyLine> segments) {
        ...
        for (PolyLine segment : segments) {
            int[] xVals = segment.xs();
            int[] yVals = segment.ys();
    ...
    private void paintLiveWire(Graphics g) {
        ...
        Point mousePos = mouseLocation.getLocation();
        PolyLine wirePath = model.liveWire(mousePos);

        int[] xVals = wirePath.xs();
        int[] yVals = wirePath.ys();
    ...
    private void paintControlPoints(Graphics g, List<Point> points) {
        ...
        for (Point p : points) {
            g.fillOval(p.x - (controlPointRadius / 2), p.y - (controlPointRadius / 2),
    ...
    private void paintMoveGuides(Graphics g, List<Point> controlPoints) {
        int selectedIndexBefore = selectedIndex - 1;
        int selectedIndexAfter = (selectedIndex + 1) % controlPoints.size();

        if (selectedIndexBefore < 0) {
            selectedIndexBefore += controlPoints.size();
        }

        g.setColor(liveWireColor);
        g.drawLine(controlPoints.get(selectedIndexBefore).x,
                controlPoints.get(selectedIndexBefore).y, mouseLocation.x, mouseLocation.y);
        g.drawLine(mouseLocation.x, mouseLocation.y, controlPoints.get(selectedIndexAfter).x,
                controlPoints.get(selectedIndexAfter).y);
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getButton() == MouseEvent.BUTTON1 && model.state().canAddPoint()) {
            model.addPoint(getMousePosition());
        } else if (e.getButton() == MouseEvent.BUTTON2 && model.state().canFinish()) {
            model.finishSelection();
        } else if (e.getButton() == MouseEvent.BUTTON3 && (model.state().canUndo())) {
            model.undoPoint();
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1 && model.state().canEdit()) {
            updateMouseLocation(e.getPoint());
            int closestIndex = model.closestPoint(mouseLocation,
                    controlPointRadius * controlPointRadius);
            if (closestIndex != -1) {
