...
public abstract class SelectionModel {
    ...
    public Point lastPoint() {
        ...
        if (state().isFinished()) {
            Point firstElem = controlPoints.getFirst();
            return new Point(firstElem.x, firstElem.y);
        }
        Point lastElem = controlPoints.getLast();
        return new Point(lastElem.x, lastElem.y);
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        for (int i = 0; i < controlPoints.size(); i++) {
            int distToPoint = (int) controlPoints.get(i).distanceSq(p);
            if (distToPoint < closest) {
                closest = distToPoint;
                closestIndex = i;
            }
        }

        if (closest > maxDistanceSq) {
            return -1;
        }
